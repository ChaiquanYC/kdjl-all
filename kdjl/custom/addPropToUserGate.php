...
switch ($act) {
    ...
    case 'addPropGG':
        ...
        if(is_array($resUserBag)){
            ...
            if($_pm['mysql']->query($sqlUserBag)){
                $error = 0;
                $userInfo = $_pm['mysql']->getOneRecord("select * from player where id='".$userID."'");
                $propInfo = $_pm['mysql']->getOneRecord("select * from props where id='".$propID."'");
                $swfData =  '恭喜玩家 ' . $userInfo['nickname'] . ' 得到GM的奖励，获得物品'.$propInfo['name']."x".$num.'!';
            ...
            if(is_array($resProp)){
                ...
                if($_pm['mysql']->query($sqlUserBag)){
                    $error = 0;
                    $userInfo = $_pm['mysql']->getOneRecord("select * from player where id='".$userID."'");
                    $swfData =  '恭喜玩家 ' . $userInfo['nickname'] . ' 得到GM的奖励，获得物品'.$resProp[0]['name']."x".$num.'!';
                    require_once(dirname(__FILE__) . '/../socketChat/config.chat.php');
                    $s = new socketmsg();
                    $s->sendMsg('an|' . $swfData);
                }else{
