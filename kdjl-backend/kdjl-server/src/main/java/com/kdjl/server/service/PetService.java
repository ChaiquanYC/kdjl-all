...
public class PetService {
    ...
    public List<Map<String, Object>> getPlayerPets(Long playerId) {
        ...
        return userPets.stream().map(up -> {
            ...
            m.put("atk", up.getAc());
            m.put("def", up.getMc());
            long maxHp = (up.getSrchp() != null ? up.getSrchp() : 0) + (up.getAddhp() != null ? up.getAddhp() : 0);
            long maxMp = (up.getSrcmp() != null ? up.getSrcmp() : 0) + (up.getAddmp() != null ? up.getAddmp() : 0);
            m.put("maxHp", maxHp);
            m.put("maxMp", maxMp);
            ...
            m.put("exp", up.getNowexp());
            m.put("maxExp", up.getLexp());
            m.put("remakelevel", up.getRemakelevel());
